...
% mat1 = C2_mat' * (C2_mat * R_x2x2 * C2_mat')^(-1) * C2_mat;
% R_ee = R_x1x1 - R_x1x2 * mat1 * R_x2x1;
% R_vv = R_ss   - R_sx2  * mat1 * R_x2s;
% R_ve = R_sx1  - R_sx2  * mat1 * R_x2x1;
% % R_ee = R_x1x1 - R_x1x2 * C2_mat' * (C2_mat * R_x2x2 * C2_mat')^(-1) * C2_mat * R_x2x1;
% % R_vv = R_ss   - R_sx2  * C2_mat' * (C2_mat * R_x2x2 * C2_mat')^(-1) * C2_mat * R_x2s;
% % R_ve = R_sx1  - R_sx2  * C2_mat' * (C2_mat * R_x2x2 * C2_mat')^(-1) * C2_mat * R_x2x1;

% temp_mat = R_ve * R_ee^(-1) * R_ve';
% [u_vec, lambda] = eig(temp_mat);

% Calculate initial values of B1, B2, C1, C2
% C1_mat = u_vec' * R_ve * R_ee^(-1);
% C2_mat = u_vec' * R_ve * R_ee^(-1);
% B1_mat = u_vec';
% B2_mat = (R_sx2 - B1_mat * C1_mat * R_x1x2) * mat1;
...
for k = 1 : max_iter
    ...
    % Calculate initial values of B1, B2, C1, C2
    C1_mat_n = u_vec' * R_ve * R_ee^(-1);
    C2_mat_n = u_vec' * R_ve * R_ee^(-1);
    B1_mat_n = u_vec';
